...
We consider bars up to homotopy equivalences (in $\S\setminus\M$) that don't change which arc of $T$ contains each endpoint.
...
Let $(\S,\M)$ be a marked surface \emph{with no punctures} and let $T$ be a triangulation.
An allowable curve $\lambda$ is \newword{compatible} with a given I-beam, ring, or bar $U$ if and only if there is an isotopy representative of $U$ (up to the isotopies/homotopies allowed in the definitions of ring, I-beam, and bar) and an \emph{honest} isotopy representative of $\lambda$ such that the representatives do not intersect each other.
...
The definition of compatibility in terms of existence of isotopy/homotopy representatives introduces a subtlety when bars are involved (precisely because there is no global prohibition against self-intersections for bars).
...
\begin{enumerate}[\qquad\rm1.]
\item \label{U to T'}
For any I-beam $U$, $\Compat(U)$ is the closure of a nonempty countable union of cones $C_{\kappa(T')}$ for near triangulations $T'$.
\item \label{T' to U}
...
For any I-beam $U$, there exist exchangeable arcs $\alpha$ and $\beta$ and isotopy representatives of $\kappa(\alpha)$ and $\kappa(\beta)$ whose union contains $U$.
\end{enumerate}
\end{cor}
\begin{proof}
Let $U$ be an I-beam.
...
The argument for Assertion~\ref{T' to U} constructs a pair $\alpha,\beta$ of exchangeable arcs and isotopy representatives of $\kappa(\alpha)$ and $\kappa(\beta)$ whose union contains an I-beam $U'$ with $C_{\kappa(T')}\subseteq\Compat(U')$.
...
\begin{enumerate}[\qquad\rm1.]
\item For any ring $U$, $\Compat(U)$ is the closure of a nonempty countable union of cones $C_{\set{U}\cup\kappa(T')}$ for pairwise compatible sets $T'$ of tagged arcs, compatible with $U$, with $|T'|=|T|-2$.
\item
Given a ring $U$ and a pairwise compatible sets $T'$ of tagged arcs compatible with $U$ with $|T'|=|T|-2$, the ring $U$ is the unique ring $U'$ (and there is no I-beam $U'$) such that $C_{\set{U}\cup\kappa(T')}\subseteq\Compat(U')$.
\end{enumerate}
\end{cor}
\begin{proof}
Suppose $U$ is a ring.
...
Given an I-beam or ring $U$, let $\arcs(U)$ be the arc sequence of $U$.
...
Proposition~\ref{basic wall} implies that the right side of Theorem~\ref{scat diag} lists the wall $(\alpha^\perp,1+\zeta_\alpha)$ exactly once for each $\alpha\in T$, and that every other wall is outgoing.
...
The definition of rings is unchanged from Definition~\ref{ring def}:  A ring in $(\S,\M)$ is an \emph{honest} \emph{non-sheilding} closed allowable curve, considered up to isotopy in $\S\setminus\M$.
