...
If $U$ is tagged plain at that puncture, then $\lambda$ is allowed to cross $U$ as it spirals into the puncture, as long as it crosses the fold edge of the triangle first, before ever crossing $U$.
If $U$ is tagged notched, it is allowed to cross $U$ as it spirals, as long as it crosses $U$ first before ever crossing the fold edge of the triangle.
...
We have supposed that \eqref{U nb} fails and councluded that \eqref{special bar nb} fails also.  %, and thus proved that \eqref{special bar nb}$\implies$\eqref{U nb}.
\end{proof}
...
Proposition~\ref{all but codim} says that $\Compat(U)$ is a convex cone, and since there are countably many cones $C_\Lambda$ and they are dense in $V^*$, the cone $\Compat(U)$ is the closure of the cones $C_\Lambda$ it contains that are full-dimensional.
Thus to prove the first assertion, we show that $\Compat(U)$ contains a cone $C_{\kappa(T')}$ for some near triangulation $T'$ and that every cone $C_\Lambda$ in $\Compat(U)$ of codimension $1$ (relative to $V^*$) has $\Lambda=T'$ for some near triangulation $T'$.
...
If $\alpha$ and $\beta$ intersect exactly once in their relative interiors and their taggings agree at all shared endpoints, then $\kappa(\alpha)$ and $\kappa(\gamma)$ are non-isotopic allowable curves that intersect exactly once (and in particular agree in spiral directions at any common endpoints.
...
If this is a non-self-folded triangle, then we place a non-leaf of 
