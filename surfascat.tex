...
A \newword{wall} is a pair $(\d,f_\d)$ such that $\d$ is a codimension-$1$ rational cone in $V^*$ with normal vector $u=\sum_{\gamma\in T}c_\gamma\gamma\in N^+$, required to be \newword{primitive} (meaning ${\gcd_{\gamma\in T}c_\gamma=1}$), and $f_\d$ is $1+\sum_{i\ge1}a_i(\zeta^u)^i$ with coefficients $a_i$ in $\k$.
...
A \newword{scattering diagram} is a collection $\D$ of walls.
There may be infinitely many walls, but if so, for each $k$ we require that only finitely many walls $(\d,f_\d)\in\D$ have $f_\d\not\equiv 1$ modulo $\m^{k+1}$.
Less formally, there are only finitely many walls $(\d,f_\d)$ such that $f_\d$ has a non-constant term of total degree $\le k$.
...
*****BELOW HERE, to Greg and Shira's stuff, is unprocessed cut-and-paste...

*****WARN the reader that we have removed the subtlety in the definition of wall-crossing that allows skew-symmetrizable stuff to work.
