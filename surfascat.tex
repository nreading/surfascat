...
\section{Nathan's attempt at barricades}
\reversemarginpar
\marginN{I'm trying to allow punctures, and I'm thinking of ``bars'' and ``rings'' in the barricade instead of ``simple barricades.''
I'm also having fewer vertices and thus longer edges.
Finally, thinking about I-beams that end in self-folded triangles at both ends has made me see that we don't want to have the graph go into a vertex and either tag it or not.  
Instead, we need to tag the edge (the one not going to a leaf) as twisted or untwisted whenever it's incident to a self-folded triangle.
(Other edges are automatically twisted).
And then we have to allow multiple edges, if they are a twisted one and an untwisted one.
This is why I wanted fewer vertices and longer edges. \\\\

This comment probably doesn't make much sense yet, but I'll try to flesh it out.
}



\normalmarginpar
\begin{definition}[\emph{Barricade}]\label{barricade def}
Let $(\S,\M)$ be a marked surface with triangulation $T$.
A \newword{barricade} in $(\S,\M)$ with respect to $T$ is a subset $\B$ \marginN{I didn't want to use a roman $B$ because it feels too much like an exchange matrix (and we have $B(T)$ already)}
of $\S$ each of whose connected components is either a \emph{closed allowable curve}, embedded honestly (in the sense of Definition~\ref{honest def}) or an \emph{embedded edge-labeled graph} in $\S\setminus\M$ such that:  \marginN{It might be better to do this using local pictures like Greg's definition.}
\begin{itemize}
\item Every edge is embedded honestly and intersects the relative interior of at least one arc in $T$ or boundary segment.
\item Embedded edges are disjoint from each other except at their endpoints and disjoint from $\partial\S$ except at leaves;
\item Every non-leaf has degree $3$,  $4$, $5$, or $6$ and is in the interior of a triangle of~$T$; \marginN{Alternately, we could make all of these degrees $3$ and get rid of multiple edges if we have three edge labels, crossed, uncrossed, or both.  That makes some things tidier but may make some things messier.}
\item Every leaf is in the relative interior of a boundary segment or an arc of $T$ and is connected by an edge to a vertex in the interior of a triangle containing the boundary segment or arc;
\item Every vertex of degree $\ge4$ is incident to exactly $3$ classes of edges under isotopy in $\S\setminus\M$;
\item Each edge is labeled ``crossed'' or ``uncrossed'';
\item An edge labeled uncrossed must connect two non-leaves, at least one of which is in the interior of a \emph{self-folded} triangle; and
\item If two edges are isotopy equivalent in $\S\setminus\M$, then one must be crossed and one must be uncrossed.
\item If the endpoints of an edge coincide, then the embedded edge must be a closed allowable curve.
\end{itemize}
We consider barricades up to isotopy equivalence (in $\S\setminus\M$) of the embedding.
Up to this equivalence, a barricade is completely determined by its abstract edge-labeled graph, which triangle contains each non-leaf, and the sequence of arcs of $T$ (and/or boundary segments) each embedded edge intersects.  \marginN{I think this is true, but I think we might have to prove it.  Alternately, we might change the definition to make it obviously true. i.e.\ "a barricade is an abstract graph together with a map from vertices to triangles and a map from edges to sequences of arcs/segments such that there exists an embedding...".}
\end{definition}
