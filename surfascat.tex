...
This is a pointed rational cone.

Given an exchangeable pair $\alpha,\beta$ of tagged arcs, there is a unique set $T'(\alpha,\beta)$ of tagged arcs such that $T'(\alpha,\beta)\cup\set{\alpha}$ is a triangulation and $T'(\alpha,\beta)\cup\set{\beta}$ is a triangulation.
We write $C_{\alpha,\beta}$ as an abbreviation for $C_{\kappa(T'(\alpha,\beta))}$.

\begin{prop}\label{Wall union}
If $U$ is an I-beam, then $\Compat$ is a union of cones $C_{\alpha,\beta}$ for exchangeable tagged arcs $\alpha$ and $\beta$.
For every exchangeable pair $\alpha,\beta$ of tagged arcs, there exists an I-beam $U$ such that $C_{\alpha,\beta}\subseteq\Compat(U)$.
\end{prop}
\begin{proof}
