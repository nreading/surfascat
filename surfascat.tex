...
The following is essentially part of \cite[Theorem~7.9]{cats1}.  \marginN{If we define $\kappa$, perhaps we do this differently.}
(That theorem contains a statement about tagged triangulations that is converted to the following statement by a bijection between tagged arcs and non-closed allowable curves defined in \cite[Definition~17.2]{cats2} or a different bijection defined in \cite[Section~5.1]{unisurface}.)

\begin{theorem}\label{card thm}
Every maximal set of non-closed allowable curves has cardinality~$|T|$.
\end{theorem}
...
\marginN{commented out here is the definition of $\kappa$.  We may need it.}
...
Given a ring or I-beam $U$, a \newword{bar in $U$} is a bar contained in $U$ that, if its endpoint is a puncture (necessarily a degenerate leaf of $U$), has the same barring as $U$ at that point.
...
\item \label{bar nb}
Every bar in $U$ is non-blocking for $\lambda$.
\item \label{special bar nb}
Every bar in $U$ that begins with a right turn and ends with a left turn or begins with a left turn and ends with a right turn is non-blocking for~$\lambda$.
\end{enumerate}
\end{lemma}
\begin{proof}
...
%\marginN{I'm inserting a new approach here that hopefully proves that $\Wall(U)$ has codimension $1$ while also proving the genteel property (in fact that every wall is gregarious).
%End of this insertion marked by Note~\ref{insert end}\label{insert}}
%\marginN{End of new insertions.  See Note~\ref{insert}\label{insert end}}
...
That is, $U$ is an honest, non-shielding, allowable curve.
...
Theorem~\ref{wall thm} also lets us prove some useful facts about $\Wall(U)$.
\begin{prop}\label{basic wall}
If $U$ is an I-beam or ring with arc sequence $\alpha_1,\ldots,\alpha_k$, then $\Wall(U)$ is a hyperplane if and only if $k=1$ if and only if $p^*(\sum_{i=1}^k\alpha_k)$ is in $\Wall(U)$.
In this case, $U$ is an I-beam.
For each arc $\gamma\in T$, there is a unique I-beam $U$ whose arc sequence has only the term~$\gamma$ so that in particular $\Wall(U)$ is the hyperplane $\gamma^\perp$.
\end{prop}
...
In this case, every bar in $U$ has either empty arc sequence or arc sequence $\alpha_1$, and Theorem~\ref{wall thm} implies that $\Wall(U)$ is a hyperplane.
...
Proposition~\ref{encirc prop}\eqref{-p relint}, says that $-p^*(\sum_{i=1}^k\alpha_k)$ is in the relative interior of $\Wall(U)$.
Since $\Wall(U)$ is convex, easy convexity arguments imply that if $\Wall(U)$ contains $p^*(\sum_{i=1}^k\alpha_k)$, then it contains a relatively open (relative to the hyperplane) neighborhood of the origin.
...
\item
At a degenerate leaf, we connect the two curves around the leaf (regardless of the plain or notched tagging of $U$).  \margin{No, there is another case here:  If the two extreme points coincide, curves at the different ends can connect to each other (one or both, depending on if its a boundary point!}
\end{itemize}
...
$\Wall(U)$ contains a cone of codimension $1$.
\item\label{p in}
$\sum_{\gamma\in T}b_\gamma(T,L^\circ(U))\gamma^*=-p^*(\sum_{i=1}^k\alpha_k)$.
\item\label{-p relint}
$-p^*(\sum_{i=1}^k\alpha_k)$ is in the relative interior of $\Wall(U)$.
\end{enumerate}
\end{prop}
\begin{proof}
...
Thus, \eqref{p in} implies \eqref{-p relint}. %that $-p^*(\sum_{i=1}^k\alpha_k)$ is in the relative interior of $\Wall(U)$.
\end{proof}




\marginN{Probably here would be a good place to point out that every codimension-$1$ cone in the mutation fan (rational quasi-lamination fan) is contained in some wall $\Wall(U)$.
And conversely that every $\Wall(U)$ is a union of codimension-$1$ cones of the mutation fan.
...
We now prove a property of walls $\Wall(U)$ that is necessary for them to constitute the walls of the cluster scattering diagram.

\begin{prop}\label{genteel}
Suppose $U$ is an I-beam or ring with arc sequence $\alpha_1,\ldots,\alpha_k$.
If $k>1$, then $p^*(\sum_{i=1}^k\alpha_i)\not\in\Wall(U)\oplus\Span_\integers\set{L^*:L\in\L}$.
\end{prop}
\begin{proof}
Checking whether $p^*(\sum_{i=1}^k\alpha_i)\in\Wall(U)\oplus\Span_\integers\set{L^*:L\in\L}$ amounts to leaving out the terms $b_\alpha(T,L)L^*$ in the definition of $p^*$ in Table~\ref{init data} and checking whether the result is in $\Wall(U)$.
