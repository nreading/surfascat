...
Need to get this in all the cases.
Call it $L^\circ(U)$
...
\item
$p^*(\sum_{i=1}^k\alpha_k)$ is in $\Wall(U)$ if and only if $\Wall(U)$ is a hyperplane.\end{enumerate}
\end{prop}
\begin{proof}
\eqref{encirc completed}:  ASSUME FOR NOW WE HAVE THIS.
...
If $U$ is an I-beam or ring with arc sequence $\alpha_1,\ldots,\alpha_k$, then $\Wall(U)$ is a hyperplane if and only if $k=1$.
