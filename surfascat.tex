...
A \newword{barricade} in $(\S,\M)$ with respect to $T$ is a subset of $\S$ each of whose connected components is either a \emph{closed allowable curve} or an \emph{embedded edge-labeled graph} in $\S\setminus\M$ such that:
\begin{itemize}
\item Every embedded edge intersects the relative interior of at least one arc in $T$ or boundary segment. 
\item No embedded edge crosses the same arc of $T$ twice without crossing a different arc of $T$ in between;  \marginN{this is the "minimal intersection" requirement}
...
\end{definition}
\includegraphics[]{cyclicA2tilde_stereo.ps}
