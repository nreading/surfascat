...
At each non-degenerate end where the extreme point is a boundary point, if we turned right to end the bar, we extend by turning left repeatedly until we reach the boundary, and if we turned left, we extend by turning right repeatedly.  \marginN{I guess we need pictures}
At each non-degenerate end where the extreme point is a puncture, we turn right or left as in the case of a boundary point, but connect the two curves.
At a degenerate leaf, we connect the two curves around the leaf.
If both extreme points are on the boundary, then $L^\circ(U)$ consists of these two curves, each with weight $1$.
Otherwise, $L^\circ(U)$ is the combined curve (which may be closed), with weight $1$.
...
Now, suppose $U$ is a ring (an honest, non-shielding, allowable curve).
...
We have proved (i).
Now \eqref{encirc codim} follows immediately:  Complete $L^\circ(U)$ as in (i) and vary the weights on the $|T|-1$ curves (and/or delete curves) to obtain the desired cone.

(iii):  ASSUME FOR NOW.

Now observe that $\sum_{\gamma\in T}b_\gamma(T,L^\circ(U))\gamma^*$ is in the relative interior of the cone constructed in the proof of (ii).
Thus, (iii) implies (iv). %that $-p^*(\sum_{i=1}^k\alpha_k)$ is in the relative interior of $\Wall(U)$.
\end{proof}
