...
%\end{itemize}
%
%when restricted to the neighborhood of a triangle in $\Delta$, the connected components are isotopic to one of pictures in Figure \ref{fig: local I-beams}.
\end{definition}
...
\begin{scope}[xshift=3cm]
	...
	\draw[red] (0,0) to (30:1);
	\draw[red] (0,0) to (270:1);
\end{scope}
\begin{scope}[xshift=6cm]
	\draw[fill=black!10, dashed] (0,0) circle (1);
	\node[marked] (1) at (90:1) {};
...
\begin{scope}[xshift=12cm]
	...
	\draw (3) to node[below] {} (1);
	\node[inner sep=0.5mm,circle,draw=red] (a) at (30:.5) {};
	\draw[red,out=210,in=-30] (a) to (150:1);
\end{scope}
\end{tikzpicture}
...
A measured lamination $\mu$ is \newword{compatible} with a barricade $B$ if there is an equivalent lamination $\mu$ which does not intersect $B$.  Two barricades are \newword{equivalent} if they are compatible with the same set of measured laminations. %Equivalence between barricades includes isotopy within the class of barricades (so degree $1$ vertices must remain on arcs in $\Delta$), as well as deleting components which do not cross any arcs in $\Delta$.

%\begin{lemma}
%Let $P$ be a proscribed graph which is connected, has no degree $3$ vertices, two degree $1$ vertices, and 
%\end{lemma}
...
\begin{itemize}
	...
	where $S_1, S_2,...,S_n$ are the shear coordinates of the arcs in $\Delta$ crossed by $B$.
