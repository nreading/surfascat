...
Comment from Fan Qin on my OCAS talk:
Qin and (Mandel?) have theta functions for the surfaces case (and theta=bracelets) in a paper that will be out soon.  

Update:  Fan Qin has proved that the theta basis is the bracelets basis.
...
Throughout, we consider arcs up to isotopy: 
Two arcs are equivalent if and only if they are related by a continuous deformation such that every stage of the deformation is an arc.
(This implies in particular that the endpoints are fixed throughout the deformation.)
...
Building cluster scattering diagrams for marked surfaces is considerably more complicated in the presence of punctures.
...
We consider bars up to isotopy in a way that is similar to I-beams:
Two bars are equivalent if they are related by a continuous deformation where every stage of the deformation is a bar, except that if the two endpoints are in the interior of the same arc of $T$, they may be moved past each other or made to coincide.
...
\begin{prop}\label{arc seq determines no punct}  \marginN{*Problem*:  The way we use this, we really need a stronger statement:  No two distinct I-beams/rings can have their arc sequences add to parallel vectors!  I think I can fix this by extending the proof given here to show that every arc sequence gives a primitive vector.}
Suppose $U$ is an I-beam or ring and $U'$ is an I-beam or ring.
If $U$ and $U'$ have the same arc sequence (as multisets), then $U=U'$.
\end{prop}
The conclusion of Proposition~\ref{arc seq determines no punct} is that $U$ equals $U'$ up to isotopy as described in Definitions~\ref{ring def} and~\ref{def: i-beam no punct}.
...
Let $(\S,\M)$ be a marked surface \emph{with no punctures} and let $T$ be a triangulation.
An allowable curve $\lambda$ is \newword{compatible} with a given I-beam, ring, or bar $U$ if and only if there is an isotopy representative of $U$ and an isotopy representative of $\lambda$ (required to be \emph{honest} as usual) such that the representatives do not intersect each other.
...
An allowable curve $\lambda$ is \newword{compatible} with a given I-beam, ring, or bar $U$ if and only \emph{for every} isotopy representative of $U$, there exists a representative of $\lambda$ (honest as usual) such that the representatives do not intersect each other.
...
\begin{proof}
Let $U$ be an I-beam.
Since $\Compat(U)$ is the closure of a union of countably many cones in $\F_\rationals(T)$ and has codimension $1$ in $V^*$, it can be written as the closure of a nonempty countable union of cones in $\F_\rationals(T)$ of codimension $1$ in $V^*$.  \marginN{Is that really right??}
...
\begin{cor}\label{wall union ring no punct} 
Let $(\S,\M)$ be a marked surface with no punctures and let $T$ be a triangulation.
\begin{enumerate}[\quad\rm\bf1.]
\item
...
%Writing $\sum\alpha$ as shorthand for $\sum_{i=1}^k\alpha_i$ and so forth, 
...
\begin{itemize}
	...
	\item If $B$ is a shielding cycle with 2 short leaves on the same side, then $C_B$ is a joint which is contained in 2 walls: the 2 I-beams contained in $B$.  \marginN{I think the correction to Proposition~\ref{closed card thm} means that this case isn't real.  UPDATE:  Maybe it is real?}
