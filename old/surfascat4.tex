...
Two arcs are \newword{compatible} if there is an isotopy representative of each such that the representatives do not intersect, except possibly at their endpoints.
...
\end{itemize}
There is no requirement that the curve not have self-intersections.  \marginN{At some point, I wrote this, but now i wonder:  Is there any reason not to rule out self-intersections in the definition of "honest"?}
\end{definition}
...
\end{itemize}
There are countably many allowable curves, up to isotopy.  \marginN{I don't think this is written down anywhere, but there is an easy argument.  Do we need to write down that argument?}
...
Every quasi-lamination can be realized with pairwise compatible curves embedded honestly.  \marginN{I don't think we need to argue that.}
...
Suppose $T$ is a triangulation and $L$ is a lamination supported on exactly $k$ closed, non-shielding allowable curves and exactly $\ell$ closed shielding allowable curves, and some number of non-closed allowable curves.
Then the number of curves in $L$ is at most $|T|-k-2\ell$.
...
We will show that $L$ has at most $|T|-k-2\ell$ curves by showing that we can add at least $2k+3\ell$ tagged arcs to $T'$ without losing pairwise compatibility.
...
Choose an honest path from $p$ to $\lambda$ and construct a tagged arc $\alpha$ starting at $p$ (tagged to agree with the taggings at $p$ of any other arcs in $T'$), following the honest path, following all the way around $\lambda$, and returning along the honest path to $p$.
...
Since we $L$ has maximal support, we see that $\alpha$ is (or $\beta$ and $\beta'$ are) in $T'$.

For each non-shielding curve $\lambda\in L$, we have shown that $T'$ contains two tagged arcs (possibly coinciding up to isotopy) that trace out an annulus surrounding $U$.
Since $S$ is compact and orientable, each shielding $\lambda$ cuts off a direct sum of tori.
We have shown that $T'$ contains one tagged arc $\alpha$ that traces along $\lambda$, with both endpoints at a point $p$.
A single torus admits a triangulation with one vertex and three arcs, so we can find at least three pairwise compatible tagged arcs from $p$ that cross $\lambda$ but are compatible with every other arc in $T'$.

When we complete $T'$ to a triangulation $T$, we must use exactly two curves for each non-shielding closed curve and at least three curves for each shielding close curve.
Thus $T'$ has at most $|T|-2k-3\ell$ tagged arcs, with equality when $\ell=0$.
Therefore the support of $L$ has at most $|T|-k-2\ell$ curves, with equality when $\ell=0$.
\end{proof}
...
\item they each have distinct endpoints, they share one or more endpoint, with their tagging disagreeing at exactly one endpoint, and they have disjoint relative interiors.
\end{itemize}
\end{proposition}
\begin{proof}
...
If $\alpha$ is an edge shared between two triangles (including in the notion of ``triangle'' a once-punctured digon formed by arcs of $T$), then $\beta$ is an arc intersecting $\alpha$ exactly once, with taggings agreeing at shared endpoints.
Otherwise, $\alpha$ is an arc, with distinct endpoints, inside a once-punctured monogon, connected to the puncture.
The other arc inside the digon either coincides with $\alpha$ except for the tagging at the puncture or is disjoint (in its relative interior) from $\alpha$ and has tagging agreeing with $\alpha$ at the puncture.
In either case, $\beta$ is an arc with distinct endpoints, disjoint from $\alpha$ in its relative interior, and disagreeing in tagging at the puncture.
...
There are four ways to do this, but some or all of them might give boundary segments rather than tagged arcs.
...
To complete the proof of the converse, suppose $\alpha$ and $\beta$ are arcs each having distinct endpoints, sharing one or more endpoint, with their tagging disagreeing at exactly one endpoint, and having disjoint relative interiors.
...
Proposition~\ref{exch char} says that $\alpha$ and $\beta$ are exchangeable if and only if $A+D=1$ when computing $(\alpha|\beta)$ \emph{and} $A+D=1$ when computing $(\beta|\alpha)$.
\end{remark}
...
\section{Walls for surfaces without punctures}
...
An \newword{I-beam} in $(\S,\M)$ is an embedded tree with two non-leaves, each connected to the other and to two leaves, satisfying the following conditions.
\begin{itemize}
...
\item The two non-leaves are in two distinct triangles and are connected to each other.
...
Pentagon as in Shira's pictures?
And/or once-punctured triangles?
\end{example}
...
Given a ring or I-beam $U$, a \newword{bar in $U$} is a bar contained in $U$.
A \newword{bar along $U$} is a bar $B$ such that there exists a homotopy (in $\S\setminus\M$) from $B$ to a (possibly self-intersecting) curve contained in $U$, that doesn't change which arc of $T$ contains each endpoint.
Since a bar is required to be honest, there exist bars \emph{along $B$} that are not (up to homotopy) \emph{in $B$} only when $U$ is a ring.
...
The \newword{arc sequence of a bar} $B$ is, essentially, the sequence of arcs of $T$ intersecting the relative interior of $B$.
...
Let $(\S,\M)$ be a marked surface \emph{with no punctures} and let $T$ be a triangulation.
An allowable curve $\lambda$ is \newword{compatible} with a given I-beam, ring, or bar $U$ if and only if there is an isotopy representative of $U$ (up to the isotopies allowed in the definitions of ring, I-beam, and bar) and an \emph{honest} isotopy representative of $\lambda$ such that the representatives do not intersect each other.
...
We first prove all of Theorem~\ref{wall thm no punct} except the codimension assertion.
...
\item \label{bar nb}
$\lambda$ is compatible with every bar along $U$.
\item \label{special bar nb}
$\lambda$ is compatible with every bar along $U$ that begins with a right turn and ends with a left turn or begins with a left turn and ends with a right turn.
\end{enumerate}
\end{lemma}
\begin{proof}
...
The bar along $U$ connecting these two exit points from interiors of triangles is not compatible with $\lambda$ and either starts with a left turn and ends with a right turn or vice versa, because otherwise, we could pass to an isotopy representative of $\lambda$ that removes the intersection.
...
If $\lambda$ is compatible with $B$, then:
...
\end{enumerate}\marginN{Notice that the if and only if statement doesn't work exactly, but that something good enough does work.}
...
Let $(\S,\M)$ be a marked surface \emph{with no punctures} and let $T$ be a triangulation.
For any ring or I-beam $U$, the set $\Compat(U)$ is a polyhedral cone in the hyperplane normal to the vector described in Theorem~\ref{wall thm no punct}, defined by the inequalities given in that theorem.
\end{prop}
\begin{proof}
...
Let $\alpha_1,\ldots,\alpha_\ell$ be the arc sequence of $B$.
...
\marginN{Move these definitions to the surfaces background section?}
...
\begin{prop}\label{Wall union no punct}
Let $(\S,\M)$ be a marked surface \emph{with no punctures}, let $T$ be a triangulation, and let $U$ be an I-beam.
Then $\Compat(U)$ is the closure of a nonempty union of cones $C_{\kappa(T')}$ for near triangulations $T'$.
\end{prop}

The key to the proof of Proposition~\ref{Wall union no punct} is the following lemma.  \marginN{***This is indeed the key, but we don't know how to prove it.***  If we get it, I suspect that we can do the full scattering diagrams story for unpunctured surfaces.  At that point, it might be possible (and maybe really complicated) to do punctured surfaces.  Or possibly with additional restrictions (e.g.\ spheres with holes cut in them) we can prove this, and then maybe extend to a punctured version (e.g.\ spheres with holes and punctures)}

\begin{lemma}\label{key}
Let $(\S,\M)$ be a marked surface \emph{with no punctures}, let $T$ be a triangulation, and let $U$ be an I-beam.
Then there exist exchangeable arcs $\alpha$ and $\beta$ and isotopy representatives of $\kappa(\alpha)$ and $\kappa(\beta)$ whose union contains $U$.
\end{lemma}
\begin{proof}  \marginN{This paragraph gives the basic strategy, although I don't know how to achieve this goal.}
The I-beam $U$ is the union of two bars, a bar $B_l$ that starts with a right turn and ends in a left turn (in either direction), and a bar $B_r$ that starts with a left turn and ends in a right turn (in either direction).
We extend both bars to non-closed allowable curves $\lambda_l$ and $\lambda_r$ that don't intersect $U$ and don't intersect each other except where they coincide along an edge of $U$.
Thus, after a suitable isotopy, $\lambda_l$ and $\lambda_r$ intersect exactly once.
...
In particular, since the curves in $\Lambda$ are embedded honestly, we obtain honest embeddings of $\lambda_l$ and $\lambda_r$.

\marginN{These next paragraphs gives a useful reduction (useless if we can't ever prove the reduced form).  There is one piece that I wonder whether it needs to be argued, but I have not argued it.  I am fairly sure it is true (sure enough for a reduction that I can't prove anyway).}
Between the two bars, there are four ends that must be extended to construct $\lambda_l$ and $\lambda_r$.
We can reduce the problem to extending just one of them honestly until it reaches the boundary.
If we can extend one, then by symmetry, we can extend all four independently, but we may end up with intersecting extensions.
Up to isotopies of the extensions that don't destroy the property of honesty, we can make the intersections occur at a finite set of points, minimize the number of intersections, and make the intersections pairwise and transverse.

We now show how to remove any intersections.
If two of the extensions intersect at a point $p$, we may as well take $p$ to be in the interior of some triangle.
Around each $p$, we construct a small open ball $\B_p$ small enough to intersect no other intersection points and no other curves, so that inside the ball there is only the single transverse intersection.
We will remove the intersections by induction on the number of balls that still contain intersections.  
(At some steps, the total number of intersections may increase, but the number of balls containing intersections will decrease, and we do not define new balls for new intersections.)

From an intersection $p$, each curve leaves the triangle through some edge of the triangle.
We follow the curves as far as they continue to pass through the same edges.
In each direction, there is some triangle that they first exit through different edges.
Consider a curve $\delta$ connecting the interiors of the two triangles, running along the two curves.
There are two possible orientations of intersections to consider.
If we orient the two extensions away from $U$ (and thus towards the boundary), either both curves follow $\delta$ in the same direction or they both follow $\delta$ in the opposite direction.

Suppose they follow $\delta$ in the same direction.
There are two ways of resolving the intersection.
One way produces a different way of extending the curves to the boundary, while the other produces an extension that connects two of the endpoints of the bars to each other and a curve that connects two boundary points.
In this case (both curves following $\delta$ in the same direction), we perform the resolution that produces extensions to the boundary, and the resulting curves are still honest.

Next suppose they follow $\delta$ in the opposite direction.
Then choose one of the extensions and delete all of it from $p$ to the boundary, instead extend it to follow parallel to the other extension back towards $U$, along $U$, and then parallel to an extension of $U$ to the boundary.
In this step, we may pick up additional intersections, but by running the new parallel curves close enough to the existing curves, we can put all of the new intersections in balls that already existed, and the ball $\B_p$ no longer contains any intersections.

In later steps, we may have, instead of two curves intersecting in the ball, two collections of parallel curves (each with mixed orientations of curves).  
These curves intersect in a grid of points, and we can consider these intersection points individually, in a suitable order, according to their orientations as in the previous paragraphs.  
\marginN{This is the part that maybe ought to be argued.  But I think it works}
Eventually, none of the balls contain intersections, to the extensions are non-intersecting, except for the one intersection inside $U$.

\marginN{NOW, we would just have to actually CONSTRUCT the extension of a given end of a bar.  
So far, no luck.  :(}
\end{proof}


\begin{proof}[Proof of Proposition~\ref{Wall union no punct}]
...
Proposition~\ref{all but codim no punct} says that $\Compat(U)$ is a convex cone, and since there are countably many cones $C_\Lambda$ and they are dense in $V^*$, the cone $\Compat(U)$ is the closure of the cones $C_\Lambda$ with $C_\Lambda\subseteq\Compat(U)$ and $\dim C_\lambda=\dim\Compat(U)$.
...
Second, we claim that every cone $C_\Lambda\subseteq\Compat(U)$ of codimension $1$ (relative to $V^*$) has $\Lambda=\kappa(T')$ for some near triangulation $T'$.

For the first claim, Lemma~\ref{key} implies that there are exchangeable arcs $\alpha$ and $\beta$ such that every allowable curve compatible with $\kappa(\alpha)$ and $\kappa(\beta)$ is also compatible with $U$.
In particular, if $T'$ is a near triangulation such that $T'\cup\set{\alpha}$ and $T'\cup\set{\beta}$ are both triangulations, every curve in $\kappa(T')$ is compatible with $U$.

To prove the second claim, Proposition~\ref{closed card thm} implies that it is enough to show that a pairwise-compatible set $\Lambda$ containing a non-shielding closed curve and having $C_\Lambda\subseteq\Compat(U)$ must have has $|\Lambda|<|T|-1$,
Given such a $\Lambda$, the non-shielding closed curve $U'$ in $\Lambda$ is a \emph{ring}, so Proposition~\ref{all but codim no punct} applies, and we see that $C_\Lambda$ is contained in the normal hyperplane to the vector determined by the arc sequence of $U'$.
But also, by Proposition~\ref{all but codim no punct}, $C_\Lambda$ is in the normal hyperplane to the vector determined by the arc sequence of $U$.
Proposition~\ref{arc seq determines} says that these are not the same hyperplane, so $\dim C_\Lambda\le|T|-2$, and thus $|\Lambda|\le|T|-2$.
\end{proof}

The following proposition is an analog of Proposition~\ref{Wall union no punct} for rings.

\begin{prop}\label{Wall union ring no punct}
Let $(\S,\M)$ be a marked surface, let $T$ be a triangulation, and let $U$ be a ring.
Then $\Compat(U)$ is the closure of a nonempty union of cones $C_{\set{U}\cup\kappa(T')}$ for pairwise compatible sets $T'$ of tagged arcs disjoint from $U$ with $|T'|=|T|-2$.
\end{prop}
\begin{proof}
Just as in the proof of Proposition~\ref{Wall union no punct}, the proof reduces to checking two claims:
First, that $\Compat(U)$ contains a cone $C_{\set{U}\cup\kappa(T')}$ as described in the statement of the proposition; and second, that every cone $C_\Lambda\subseteq\Compat(U)$ of codimension $1$ (relative to $V^*$) is of the form $C_{\set{U}\cup\kappa(T')}$.

To prove the first claim, we repeat a construction from the proof of Proposition~\ref{closed card thm}, with $U$ replacing $\lambda$.
On each side of $U$, we construct an arc $\alpha$ (or a pair of arcs $\beta$ and $\beta'$) such that the curves on both sides cut out an annulus surrounding.
These curves can be completed to a triangulation containing exactly $2$ tagged arcs that are not disjoint from $U$.  
Deleting those two arcs from the triangulation, we obtain the desired $T'$.

The second claim follows immediately from Proposition~\ref{closed card thm}.
\end{proof}

Propositions~\ref{Wall union no punct} and~\ref{Wall union ring no punct} say in particular that for any ring or I-beam $U$, the set $\Compat(U)$ contains a cone of codimension $1$ in $V^*$.
Together with Proposition~\ref{all but codim no punct}, this completes the proof of Theorem~\ref{wall thm no punct}.

The following proposition is a form of converse to Proposition~\ref{Wall union no punct}.

\begin{prop}\label{near I no punct}
Let $(\S,\M)$ be a marked surface \emph{with no punctures} and let $T$ be a triangulation.
...
\end{prop}
\begin{proof}
Since $(\S,\M)$ has no punctures, we deal only in ordinary arcs (or equivalently, all tags are plain).
Suppose $T'$ is a near triangulation and let $\alpha$ and $\beta$ be the arcs that (separately) complete $T'$ to a triangulation.
By Proposition~\ref{exch char} and the fact that $(\S,\M)$ has no punctures, $\alpha$ and $\beta$ intersect exactly once in their relative interiors.
We will construct an I-beam $U$ and verify that $C_{\kappa(T')}\subseteq\Compat(U)$.
...
The allowable curves $\kappa(\alpha)$ and $\kappa(\gamma)$ are non-isotopic and intersect exactly once.  \marginN{I stole some of this language to use in the (still incomplete) proof of Lemma~\ref{key}.  So if that remains in that lemma, I should revise on of the two to refer to the other.}
...
We can choose a subset of $\kappa(\alpha)$ that is a bar $B_\alpha$ and a subset of $\kappa(\beta)$ that is a bar $B_\beta$ such that there are isotopy representatives of $B_\alpha$ and $B_\beta$ whose union is $U$.
Thus any allowable curve compatible with $\kappa(\alpha)$ and $\kappa(\beta)$ is also compatible with~$U$.
...
The following is the analog, for rings, of Proposition~\ref{near I no punct}, a form of converse to Proposition~\ref{Wall union ring no punct}.

\begin{prop}\label{ring face no punct}
Let $(\S,\M)$ be a marked surface \emph{with no punctures} and let $T$ be a triangulation.
Given a ring $U$ and a pairwise compatible sets $T'$ of tagged arcs disjoint from $U$ with $|T'|=|T|-2$, the ring $U$ is the unique ring $U'$ (and there is no I-beam $U'$) such that $C_{\set{U}\cup\kappa(T')}\subseteq\Compat(U')$.
\end{prop}
\begin{proof}
The allowable curve $U$ is compatible with the ring $U$, so the containment $C_{\set{U}\cup\kappa(T')}\subseteq\Compat(U')$ holds by definition.
The uniqueness (and ruling our an I-beam $U'$) holds by Proposition~\ref{arc seq determines no punct} and the fact that $C_{\set{U}\cup\kappa(T')}$ has codimension $1$ in $V^*$.
\end{proof}




\section{A holding place for the rest of the material that I want to clean up in the no-punctures case}


Theorem~\ref{wall thm no punct} also lets us prove some useful facts about $\Compat(U)$.
\begin{prop}\label{basic wall}
If $U$ is an I-beam or ring with arc sequence $\alpha_1,\ldots,\alpha_k$, then $\Compat(U)$ is a hyperplane if and only if $k=1$ if and only if $p^*(\sum_{i=1}^k\alpha_k)$ is in $\Compat(U)$.
...
Proposition~\ref{encirc prop}\eqref{-p relint}, says that $-p^*(\sum_{i=1}^k\alpha_k)$ is in the relative interior of $\Compat(U)$.
Since $\Compat(U)$ is convex, easy convexity arguments imply that if $\Compat(U)$ contains $p^*(\sum_{i=1}^k\alpha_k)$, then it contains a relatively open (relative to the hyperplane) neighborhood of the origin.
...
\item\label{p in}
$\sum_{\gamma\in T}b_\gamma(T,L^\circ(U))\gamma^*=-p^*(\sum_{i=1}^k\alpha_k)$.
\item\label{-p relint}
$-p^*(\sum_{i=1}^k\alpha_k)$ is in the relative interior of $\Compat(U)$.
\end{enumerate}
\end{prop}
\begin{proof}
...
Thus, \eqref{p in} implies \eqref{-p relint}. %that $-p^*(\sum_{i=1}^k\alpha_k)$ is in the relative interior of $\Compat(U)$.
\end{proof}
...
First, suppose $U$ is a non-degenerate I-beam with arc sequence $\alpha_1,\ldots,\alpha_k$.
Consider the example of Figure~\ref{genteel}, in which $k=7$.
\begin{figure}
\scalebox{0.7}{\includegraphics{genteel}}
\caption{Computing $p^*(\sum_{i=1}^k\alpha_i)$ for a non-degenerate I-beam}
\label{genteel}
\end{figure}
Each $+$-sign on an arc $\gamma$ in the figure represents a term $\gamma^*$ present in $p^*(\sum_{i=1}^k\alpha_i)$.
The figure is completely representative (of the non-degenerate case) if we keep in mind that some arcs shown may actually be boundary segments and that some pairs of arcs shown may coincide.  
(However, this cannot be the case for any of the arcs in the arc-sequence of $U$.)
We see that if $k>1$, then the coefficient of $\alpha_1$
...
******\marginN{Below here, the case with punctures allowed, which is still a mess.}
******
...
\begin{definition}[\emph{I-beam}]\label{def: i-beam}
...
\marginN{Note that we do not have the case of edges connecting self-folded triangles to each other.  Turns out that those are lower-dimensional!  UPDATE:  I was wrong about that, so I have changed this definition. 
A LITTLE MORE DETAIL:  What is lower dimensional is when this is accomplished by curves that have disagreeing spirals at both endpoints.  But there are other ways this can happen.}
\marginN{As I'm proving Proposition~\ref{}, I'm seeing that this definition needs to be broadened further:  We need to allow all punctures to be degenerate vertices.  BUT I HAVE NOT MADE THIS CHANGE YET!!}
An \newword{I-beam} in $(\S,\M)$ is an embedded tree with $0$, $1$, or $2$ non-leaves, satisfying conditions and possibly having markings as follows.
\begin{itemize}
\item Every edge is an honest, non-self-intersecting curve.
\item Embedded edges are disjoint from each other except possibly at shared endpoints and disjoint from $\partial\S$ except possibly at leaves.
...
\item If the graph has no non-leaves, then it has two leaves, located at distinct punctures, each inside a self-folded triangle, and connected to each other by an edge.
\item 
The edge connected to a leaf at a puncture is ``tagged'' either \newword{plain} \newword{notched} at the puncture.  
Notched taggings are indicated with $\notch$ in drawings.
\end{itemize}
We consider I-beams up to isotopy equivalences (in $\S\setminus\M$) of the embedding that don't change which arc of $T$ or boundary segment of $(\S,\M)$ or puncture contains each leaf.
(If two leaves are in the interior of the same arc of $T$, necessarily connected to two different non-leaves in two different triangles, we allow isotopies that move them past each other.)  \marginN{Added this important parenthetical}
The \newword{relative interior} of an I-beam is the I-beam minus its leaves.
...
\begin{definition}[\emph{Bar}]\label{bar def}
A \newword{bar} in $(\S,\M)$ is an \emph{honest} curve $\gamma:[0,1]\to\S$ each of whose endpoints is on an arc of $T$ (not the fold edge of a self-folded triangle) \marginN{added parenthetical}
or a boundary segment of $(\S,\M)$ or is the puncture in a self-folded triangle.
The \newword{relative interior} of $\gamma$ is the restriction of $\gamma$ to $(0,1)$.  
...
We consider bars up to isotopy equivalences (in $\S\setminus\M$) that don't change which arc of $T$ contains each endpoint (or don't move the endpoint if it is a puncture).
We will be most interested in bars contained in some I-beam or ring.

\marginN{Made a change in here to reflect Greg's correction (embedded or immersed).  This is going to worsen one of the things Greg complained about:  an over-abundance of terms.  I think he's probably right, but I want to get a complete mathematical story written before we think hard about how to fix the problem.}
...
A \newword{bar along $U$} is a bar $B$ such that there exists a homotopy (in $\S\setminus\M$) from $B$ to a (possibly self-intersecting) curve contained in $U$, that don't change which arc of $T$ contains each endpoint (or don't move the endpoint if it is a puncture), with the same condition on tagging.
Since a bar is required to be honest, there exist bars \emph{along $B$} that are not (up to homotopy) \emph{in $B$} only when $U$ is a ring.
...
\begin{definition}[\emph{Arc sequence of a ring, I-beam, or bar}]\label{seq def}  
The arc sequences we define here might instead be called ``arc-multisets,'' because we only care about the underlying multiset and because there will be some ambiguities in the ordering.
However, the term ``arc sequence'' seems more pleasant and also more correctly captures the spirit of the definition despite the ambiguities.
...
Second, if $\alpha$ is the non-fold edge of some self-folded triangle and $\beta$ is the fold edge, any occurrence of adjacent entries $\alpha,\beta,\alpha$ in the sequence is replaced by $\alpha,\beta$.
...
The \newword{arc sequence of a ring} $U$ is obtained from the cycle of arcs of $T$ intersecting $U$ by applying the second of the two modifications above and then cutting the cycle arbitrarily to make a sequence.
\end{definition}
